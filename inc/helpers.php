...
function wcu_get_dashicon_svg( $name, $args = array() ) {
	...
	// Graceful fallback: if a wp.org badge uses a dashicon we haven't
	// bundled yet, render the awards star instead of an empty disc.
	// New badges added on wp.org will still show a colored ring + star
	// without anyone needing to add the path here first.
	if ( ! isset( $paths[ $name ] ) ) {
		$name = 'awards';
	}
